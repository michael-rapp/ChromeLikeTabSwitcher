...
public class PhoneTabSwitcherLayout extends AbstractTabSwitcherLayout
    ...
    private int calculateBottomMargin(@NonNull final AbstractItem item) {
        ...
        float tabHeight =
                (view.getHeight() - 2 * tabInset) * getArithmetics().getTabScale(item, true);
    ...
    private void animateShowSwitcher(@NonNull final AbstractItem item, final long duration,
        ...
        getArithmetics().setTabScale(Axis.DRAGGING_AXIS, item, 1);
        getArithmetics().setTabScale(Axis.ORTHOGONAL_AXIS, item, 1);
        getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, item,
                getArithmetics().getTabPivot(Axis.DRAGGING_AXIS, item, DragState.NONE));
        getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item,
                getArithmetics().getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.NONE));
        float scale = getArithmetics().getTabScale(item, true);
        int selectedTabIndex = getModel().getSelectedTabIndex();

        if (item.getIndex() < selectedTabIndex) {
            getArithmetics().setTabPosition(Axis.DRAGGING_AXIS, item,
        ...
        getArithmetics().animateTabScale(Axis.DRAGGING_AXIS, animation, scale);
        getArithmetics().animateTabScale(Axis.ORTHOGONAL_AXIS, animation, scale);
        getArithmetics().animateTabPosition(Axis.DRAGGING_AXIS, animation, item,
                item.getTag().getPosition(), true);
        getArithmetics().animateTabPosition(Axis.ORTHOGONAL_AXIS, animation, item, 0, true);
    ...
    private void animateHideSwitcher() {
        ...
        while ((item = iterator.next()) != null) {
            if (item.isInflated()) {
                ...
                inflateAndUpdateView(item, createHideSwitcherLayoutListener(item));
            }
        }
    ...
    private void animateHideSwitcher(@NonNull final AbstractItem item, final long duration,
        ...
        animation.setInterpolator(interpolator);
        animation.setListener(new AnimationListenerWrapper(listener));
        getArithmetics().animateTabScale(Axis.DRAGGING_AXIS, animation, 1);
        getArithmetics().animateTabScale(Axis.ORTHOGONAL_AXIS, animation, 1);
        FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) view.getLayoutParams();
        getArithmetics().animateTabPosition(Axis.ORTHOGONAL_AXIS, animation, item,
    ...
    private void animateSwipe(@NonNull final AbstractItem item, final boolean remove,
                              ...
                              @Nullable final AnimatorListener listener) {
        View view = item.getView();
        float currentScale = getArithmetics().getTabScale(item, true);
        ...
        float targetPosition = remove ?
                (swipeAnimation.getDirection() == SwipeDirection.LEFT ? -1 * swipePosition :
                        swipePosition) : 0;
        float currentPosition = getArithmetics().getTabPosition(Axis.ORTHOGONAL_AXIS, item);
        ...
        getArithmetics()
                .animateTabPosition(Axis.ORTHOGONAL_AXIS, animation, item, targetPosition, true);
        getArithmetics().animateTabScale(Axis.ORTHOGONAL_AXIS, animation,
                remove ? swipedTabScale * currentScale : currentScale);
        getArithmetics().animateTabScale(Axis.DRAGGING_AXIS, animation,
                remove ? swipedTabScale * currentScale : currentScale);
    ...
    private void animateSwipe(@NonNull final TabItem tabItem, final float targetPosition,
        ...
        float currentPosition = getArithmetics().getTabPosition(Axis.X_AXIS, tabItem);
        float distance = Math.abs(targetPosition - currentPosition);
        float maxDistance = getArithmetics().getTabSize(Axis.X_AXIS, tabItem) + swipedTabDistance;
        ...
        animation.setDuration(duration);
        animation.setStartDelay(0);
        getArithmetics().animateTabPosition(Axis.X_AXIS, animation, tabItem, targetPosition, true);
        animation.start();
    }
    ...
    private void animateRemove(@NonNull final AbstractItem removedItem,
        ...
        getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, removedItem,
                getArithmetics().getTabPivot(Axis.DRAGGING_AXIS, removedItem, DragState.SWIPE));
        getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, removedItem,
                getArithmetics().getTabPivot(Axis.ORTHOGONAL_AXIS, removedItem, DragState.SWIPE));
        animateSwipe(removedItem, true, 0, swipeAnimation,
                createRemoveAnimationListener(removedItem));
    }
    ...
    private void animateRelocate(@NonNull final AbstractItem item, final float position,
        ...
        getArithmetics().animateTabPosition(Axis.DRAGGING_AXIS, animation, item, position, true);
        animation.setStartDelay(delay);
        animation.start();
    }
    ...
    private void animateRevertStartOvershoot(@NonNull final Interpolator interpolator) {
        ...
        getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item,
                getArithmetics().getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.NONE));
        float position = getArithmetics().getTabPosition(Axis.DRAGGING_AXIS, item);
        float targetPosition = item.getTag().getPosition();
        final float startPosition = getArithmetics().getTabPosition(Axis.DRAGGING_AXIS, item);
            ...
            public void onAnimationUpdate(final ValueAnimator animation) {
                ...
                while ((item = iterator.next()) != null) {
                    if (item.getIndex() == 0) {
                        ...
                        View view = item.getView();
                        view.setVisibility(
                                getArithmetics().getTabPosition(Axis.DRAGGING_AXIS, firstItem) <=
                                        getArithmetics().getTabPosition(Axis.DRAGGING_AXIS, item) ?
    ...
    private boolean animateTilt(@NonNull final Interpolator interpolator, final float maxAngle,
        ...
        while ((item = iterator.next()) != null) {
            if (item.isInflated() &&
                    getArithmetics().getTabRotation(Axis.ORTHOGONAL_AXIS, item) != 0) {
                View view = item.getView();
                ViewPropertyAnimator animation = view.animate();
                animation.setListener(new AnimationListenerWrapper(
                ...
                animation.setDuration(Math.round(revertOvershootAnimationDuration *
                        (Math.abs(getArithmetics().getTabRotation(Axis.ORTHOGONAL_AXIS, item)) /
                                maxAngle)));
                animation.setInterpolator(interpolator);
                getArithmetics().animateTabRotation(Axis.ORTHOGONAL_AXIS, animation, 0);
                animation.setStartDelay(0);
                animation.start();
                result = true;
            }
    ...
    private void animatePeek(@NonNull final AbstractItem item, final long duration,
        ...
        FrameLayout.LayoutParams layoutParams = (FrameLayout.LayoutParams) view.getLayoutParams();
        view.setAlpha(1f);
        getArithmetics().setTabPivot(Axis.X_AXIS, item, x);
        getArithmetics().setTabPivot(Axis.Y_AXIS, item, y);
        view.setX(layoutParams.leftMargin);
        view.setY(layoutParams.topMargin);
        getArithmetics().setTabScale(Axis.DRAGGING_AXIS, item, 0);
        getArithmetics().setTabScale(Axis.ORTHOGONAL_AXIS, item, 0);
        ViewPropertyAnimator animation = view.animate();
        animation.setInterpolator(interpolator);
        ...
        getArithmetics().animateTabScale(Axis.ORTHOGONAL_AXIS, animation, 1);
        getArithmetics()
                .animateTabPosition(Axis.DRAGGING_AXIS, animation, item, peekPosition, true);
        animation.start();
        int selectedTabIndex = getModel().getSelectedTabIndex();
    ...
    private OnGlobalLayoutListener createRelocateLayoutListener(@NonNull final AbstractItem item,
        ...
        return new OnGlobalLayoutListener() {
            ...
            @Override
            public void onGlobalLayout() {
                animateRelocate(item, position, tag, delay, listener);
            }
            ...
            public void onGlobalLayout() {
                ...
                if (count - addedItems.length == 0) {
                    ...
                    if (state == null || state == State.STACKED_START) {
                        items = relocateWhenAddingStackedTabs(true, addedItems);
                    } else if (state == State.STACKED_END) {
                        items = relocateWhenAddingStackedTabs(false, addedItems);
                ...
                for (AbstractItem item : items) {
                    ...
                    if (previousTag == null || tag.getPosition() != previousTag.getPosition()) {
                        ...
                        float swipePosition = calculateSwipePosition();
                        float scale = getArithmetics().getTabScale(item, true);
                        getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, item, getArithmetics()
                                .getTabPivot(Axis.DRAGGING_AXIS, item, DragState.NONE));
                        getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item, getArithmetics()
                                .getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.NONE));
                        getArithmetics()
                                .setTabPosition(Axis.DRAGGING_AXIS, item, tag.getPosition());
                        getArithmetics().setTabPosition(Axis.ORTHOGONAL_AXIS, item,
                                swipeAnimation.getDirection() == SwipeDirection.LEFT ?
                                        -1 * swipePosition : swipePosition);
                        getArithmetics().setTabScale(Axis.DRAGGING_AXIS, item, scale);
                        getArithmetics().setTabScale(Axis.ORTHOGONAL_AXIS, item, scale);
                        getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, item, getArithmetics()
                                .getTabPivot(Axis.DRAGGING_AXIS, item, DragState.SWIPE));
                        getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item, getArithmetics()
                                .getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.SWIPE));
                        getArithmetics()
                                .setTabScale(Axis.DRAGGING_AXIS, item, swipedTabScale * scale);
                        getArithmetics()
                                .setTabScale(Axis.ORTHOGONAL_AXIS, item, swipedTabScale * scale);
    ...
    private OnGlobalLayoutListener createSwipeNeighborLayoutListener(
        ...
        return new OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                getArithmetics().setTabPosition(Axis.X_AXIS, neighbor, position);
            }
     ...
     */
    @NonNull
    private AnimatorListener createRemoveAnimationListener(
            @NonNull final AbstractItem removedItem) {
        return new AnimatorListenerAdapter() {
            ...
            public void onAnimationStart(final Animator animation) {
                ...
                if (state == State.STACKED_END) {
                    relocateWhenRemovingStackedTab(removedItem, false);
                } else if (state == State.STACKED_START) {
                    relocateWhenRemovingStackedTab(removedItem, true);
            ...
            public void onAnimationEnd(final Animator animation) {
                ...
                Interpolator interpolator =
                        peekAnimation.getInterpolator() != null ? peekAnimation.getInterpolator() :
                                new AccelerateDecelerateInterpolator();
                View view = item.getView();
                getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, item, tabTitleContainerHeight);
                getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item,
                        getArithmetics().getTabSize(Axis.ORTHOGONAL_AXIS, item) / 2f);
                ...
                animator.alpha(0);
                getArithmetics().animateTabPosition(Axis.DRAGGING_AXIS, animator, item,
                        getArithmetics().getTabPosition(Axis.DRAGGING_AXIS, item) * 1.5f);
                getArithmetics().animateTabScale(Axis.DRAGGING_AXIS, animator, 0);
                getArithmetics().animateTabScale(Axis.ORTHOGONAL_AXIS, animator, 0);
                animator.start();
            }
    ...
    private void relocateWhenRemovingFloatingTab(@NonNull final AbstractItem removedItem,
        ...
        if (removedItem.getIndex() > 0) {
            ...
            while ((item = iterator.next()) != null) {
                ...
                if (tag.getState() != State.HIDDEN) {
                    long startDelay = Math.abs(removedItem.getIndex() - item.getIndex()) *
                    ...
                    relocate(item, tag.getPosition(), tag, startDelay);
                } else {
                    break;
                }
            }
        ...
        if (attachedPositionChanged && getModel().getCount() > 2 &&
            ...
            while ((item = iterator.next()) != null &&
                ...
                tag.setPosition(pair.first);
                tag.setState(pair.second);
                long startDelay = (Math.abs(removedItem.getIndex() - item.getIndex()) + 1) *
                        relocateAnimationDelay;
    ...
    private void relocateWhenRemovingStackedTab(@NonNull final AbstractItem removedItem,
        ...
        while ((item = iterator.next()) != null && (item.getTag().getState() == State.HIDDEN ||
            ...
            if (item.getTag().getState() == State.HIDDEN) {
                ...
                long startDelay =
                        (Math.abs(startIndex - item.getIndex()) + 1) * relocateAnimationDelay;
                animateRelocate(item, previousProjectedPosition, null, startDelay,
                        createRelocateAnimationListener(item));
            }
        ...
        for (AbstractItem addedItem : addedItems) {
            ...
            while ((item = iterator.next()) != null) {
                ...
                if (item.getIndex() >= firstAddedItem.getIndex() &&
                    ...
                    relocate(item, tag.getPosition(), tag, 0);
                }
    ...
    private void relocate(@NonNull final AbstractItem item, final float relocatePosition,
        ...
        if (item.isInflated()) {
            animateRelocate(item, relocatePosition, tag, startDelay,
                    createRelocateAnimationListener(item));
        } else {
            inflateAndUpdateView(item,
                    createRelocateLayoutListener(item, relocatePosition, tag, startDelay,
                            createRelocateAnimationListener(item)));
    ...
    private void swipe(@NonNull final AbstractItem item, final float distance) {
        ...
        getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, item,
                getArithmetics().getTabPivot(Axis.DRAGGING_AXIS, item, DragState.SWIPE));
        getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item,
                getArithmetics().getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.SWIPE));
        float scale = getArithmetics().getTabScale(item, true);
        ...
        getArithmetics().setTabScale(Axis.DRAGGING_AXIS, item, targetScale);
        getArithmetics().setTabScale(Axis.ORTHOGONAL_AXIS, item, targetScale);
        view.setAlpha(swipedTabAlpha + ratio * (1 - swipedTabAlpha));
        getArithmetics().setTabPosition(Axis.ORTHOGONAL_AXIS, item, dragDistance);
    }
    ...
    private void startOvershoot(final float position) {
        ...
        while ((item = iterator.next()) != null) {
            if (item.getIndex() == 0) {
                ...
                getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item,
                        getArithmetics().getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.NONE));
                getArithmetics().setTabPosition(Axis.DRAGGING_AXIS, item, position);
            } else if (item.isInflated()) {
                AbstractItem firstItem = iterator.first();
                View view = item.getView();
                view.setVisibility(getArithmetics().getTabPosition(Axis.DRAGGING_AXIS, firstItem) <=
                        getArithmetics().getTabPosition(Axis.DRAGGING_AXIS, item) ? View.INVISIBLE :
                        View.VISIBLE);
            }
        }
    }
    ...
    private void tiltOnStartOvershoot(final float angle) {
        ...
        while ((item = iterator.next()) != null) {
            ...
            if (item.getIndex() == 0) {
                ...
                getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, item, getArithmetics()
                        .getTabPivot(Axis.DRAGGING_AXIS, item, DragState.OVERSHOOT_START));
                getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item, getArithmetics()
                        .getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.OVERSHOOT_START));
                getArithmetics().setTabRotation(Axis.ORTHOGONAL_AXIS, item, angle);
            } else if (item.isInflated()) {
                item.getView().setVisibility(View.INVISIBLE);
            }
        }
    ...
    private void tiltOnEndOvershoot(final float angle) {
        ...
        while ((item = iterator.next()) != null) {
            if (item.isInflated()) {
                ...
                getArithmetics().setTabPivot(Axis.DRAGGING_AXIS, item, getArithmetics()
                        .getTabPivot(Axis.DRAGGING_AXIS, item, DragState.OVERSHOOT_END));
                getArithmetics().setTabPivot(Axis.ORTHOGONAL_AXIS, item, getArithmetics()
                        .getTabPivot(Axis.ORTHOGONAL_AXIS, item, DragState.OVERSHOOT_END));
                getArithmetics().setTabRotation(Axis.ORTHOGONAL_AXIS, item, angle);
            }
        }
    }
    ...
    protected final void updateView(@NonNull final AbstractItem item) {
        ...
        getArithmetics().setTabPivot(Arithmetics.Axis.DRAGGING_AXIS, item, getArithmetics()
                .getTabPivot(Arithmetics.Axis.DRAGGING_AXIS, item,
                        AbstractDragEventHandler.DragState.NONE));
        getArithmetics().setTabPivot(Arithmetics.Axis.ORTHOGONAL_AXIS, item, getArithmetics()
                .getTabPivot(Arithmetics.Axis.ORTHOGONAL_AXIS, item,
                        AbstractDragEventHandler.DragState.NONE));
        super.updateView(item);
        getArithmetics().setTabRotation(Arithmetics.Axis.ORTHOGONAL_AXIS, item, 0);
    }
    ...
    protected final void inflateAndUpdateView(@NonNull final AbstractItem item,
        ...
        inflateView(item, createInflateViewLayoutListener(item, listener), tabViewBottomMargin);
    }
    ...
    public final void onTabRemoved(final int index, @NonNull final Tab tab,
        ...
        if (!getModel().isSwitcherShown()) {
            ...
            if (removedItem.isInflated()) {
                ...
                inflateAndUpdateView(removedItem,
    ...
    public final void onAllTabsRemoved(@NonNull final Tab[] tabs,
        ...
        if (!getModel().isSwitcherShown()) {
            ...
            while ((item = iterator.next()) != null) {
                ...
                if (item.isInflated()) {
                    animateSwipe(item, true, startDelay, swipeAnimation,
    ...
    public final void onSwipeEnded(@NonNull final TabItem tabItem, final boolean remove,
        ...
        if (remove) {
            SwipeDirection direction =
                    getArithmetics().getTabPosition(Axis.ORTHOGONAL_AXIS, tabItem) < 0 ?
    ...
    public final void onSwitchingBetweenTabs(final int selectedTabIndex, final float distance) {
        ...
        if (distance == 0 || (distance > 0 && selectedTabIndex < getModel().getCount() - 1) ||
            ...
            getArithmetics().setTabPosition(Axis.X_AXIS, tabItem, distance);
            float position = getArithmetics().getTabPosition(Axis.X_AXIS, tabItem);
            ...
            getArithmetics().setTabPosition(Axis.X_AXIS, tabItem, position);
        }
    ...
    public final void onSwitchingBetweenTabsEnded(final int selectedTabIndex,
        ...
        } else if (getArithmetics().getTabPosition(Axis.X_AXIS, selectedTabItem) > 0) {
        ...
        if (neighbor != null && neighbor.isInflated()) {
            float width = getArithmetics().getTabSize(Axis.X_AXIS, neighbor);
